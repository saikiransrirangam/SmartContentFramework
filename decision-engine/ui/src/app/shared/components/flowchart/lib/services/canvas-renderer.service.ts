import { ChangeDetectorRef, ComponentRef, Injectable, ViewContainerRef } from '@angular/core';
...
} from '../ng-flowchart-connector/ng-flowchart-connector.component';
...
export class CanvasRendererService {
	...
	constructor(private options: OptionsService, private cdr: ChangeDetectorRef) {}
	...
	public renderConnector(connector: ComponentRef<NgFlowchartConnectorComponent>) {
	...
	public render(flow: CanvasFlow, pretty?: boolean, skipAdjustDimensions?: boolean) {
		if (!flow.hasRoot()) {
	...
	private adjustDimensions(flow: CanvasFlow, canvasRect: DOMRect): void {
		...
		const widthDiff =
			canvasRect.width / this.scale - (maxRight - canvasRect.left + widthBorderGap);
		if (widthDiff < -this.getStepGap()) {
			...
			if (this.isNestedCanvas()) {
				const shrinkWidth =
					this.options.options.orientation === 'VERTICAL' ? widthDiff * 2 : widthDiff;
				this.getCanvasContentElement().style.minWidth = `${
		...
		const heightDiff =
			canvasRect.height / this.scale - (maxBottom - canvasRect.top + heightBorderGap);
		if (heightDiff < -this.getStepGap()) {
			...
			if (this.isNestedCanvas()) {
				const shrinkHeight =
					this.options.options.orientation === 'HORIZONTAL' ? heightDiff * 2 : heightDiff;
				this.getCanvasContentElement().style.minHeight = `${
	...
	private setRootPosition(step: NgFlowchartStepComponent, dragEvent?: DragEvent) {
		if (!dragEvent) {
			...
			step.zsetPosition([10, canvasCenter[1]], true);
			return;
		}
	...
	private isNestedCanvas(): boolean {
		if (this.viewContainer) {
			const canvasWrapper = (this.viewContainer.element.nativeElement as HTMLElement)
				.parentElement;
			if (canvasWrapper) {
		...
		for (const conn of flow.connectors) {
			...
			if (this.options.options.orientation === 'VERTICAL') {
				startStepPos = [
					...
					startStepRect.top - canvasRect.top + startStepRect.height / this.scale,
				];
			} else if (this.options.options.orientation === 'HORIZONTAL') {
				startStepPos = [
					startStepRect.left - canvasRect.left + startStepRect.width / this.scale,
					startStepRect.top -
						canvasRect.top +
	...
	private findClosestEndEdge(startPos: number[], stepRect: Partial<DOMRect>): number[] {
		let sides: number[][];
