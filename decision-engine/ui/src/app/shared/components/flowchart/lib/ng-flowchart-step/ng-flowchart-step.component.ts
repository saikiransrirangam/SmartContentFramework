import {
    AfterViewInit, Component, ComponentRef, ElementRef, EventEmitter, HostListener, Injector, Input,
    OnInit, Output, TemplateRef, ViewChild, ViewContainerRef, ViewEncapsulation
} from '@angular/core';
...
} from '../ng-flowchart-connector-pad/ng-flowchart-connector-pad.component';
...
export class NgFlowchartStepComponent<T = any> implements OnInit, AfterViewInit {
	...
	isConnectorPadEnabled(): boolean {
		...
		return !(
			...
			(this.hasChildren() ||
				this.canvas.flow.connectors.filter(c => c.connector.startStepId === this.id)
					.length > 0)
		);
	}
	...
	private destroy0(parentIndex, recursive: boolean = true) {
		...
		const connectors = this.canvas.flow.connectors.filter(
			c => c.connector.startStepId === this.id || c.connector.endStepId === this.id
		);
		for (const conn of connectors) {
			conn.destroy0();
		}
	...
	drawConnectorPad(position: number[]): void {
		...
		const hidePad =
			this.canvas.disabled || !this.isConnectorPadEnabled() || this.isRootElement();
	...
	private internalIsValidConnectorDropTarget(): boolean {
		...
		const stepsInSameCanvas =
			this.canvas.flow.steps.some(s => s.id === this.drop.dragConnector.startStepId) &&
