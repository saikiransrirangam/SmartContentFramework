...
export class NgFlowchartCanvasDirective implements OnInit, OnDestroy, AfterViewInit {
	...
	protected canvasDragScroll(e: MouseEvent) {
		var validDragAnchor =
			e.target === this.canvasContent || e.target === this.canvasEle.nativeElement;
		const validLeftClick =
			this.options.dragScroll.includes('LEFT') && validDragAnchor && e.button === 0;
		const validOther =
			(this.options.dragScroll.includes('MIDDLE') && e.button === 1) ||
			(this.options.dragScroll.includes('RIGHT') && e.button === 2);
		const scrollable =
			this.canvasEle.nativeElement.scrollWidth > this.canvasEle.nativeElement.clientWidth ||
			this.canvasEle.nativeElement.scrollHeight > this.canvasEle.nativeElement.clientHeight;
